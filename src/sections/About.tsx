...
export function About() {
  return (
    <section id="about" className="relative overflow-hidden py-24 sm:py-32">
      <BigType text="ABOUT" className="-top-10 text-ivory/[0.05]" parallax={-50} />
        ...
        <RevealGroup className="mx-auto mt-16 grid max-w-5xl grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" stagger={0.07}>
          {aboutContent.map((block) => (
            <RevealItem key={block.id}>
              <div className="h-full rounded-2xl border border-ivory/10 bg-charcoal-soft/50 p-8 shadow-[0_20px_50px_-30px_rgba(0,0,0,0.5)] backdrop-blur-sm">
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-royal/15 text-royal-light">
                  <Icon name={block.icon} className="h-5 w-5" />
                </div>
                <h3 className="mt-4 font-display text-lg font-medium text-ivory">{block.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-ivory/60">{block.body}</p>
              </div>
            </RevealItem>
          ))}
        </RevealGroup>
